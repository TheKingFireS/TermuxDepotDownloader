...
installersetup() {
	...
	echo "permit nopass :wheel as root" > "$installed_rootfs"/rootfs/etc/doas.d/doas.conf
...
dlfile() {
	...
	unzip -j DepotDownloader.zip "DepotDownloader" -d "$installed_rootfs"/rootfs/usr/local/bin
	rm DepotDownloader.zip
	chmod u+x "$installed_rootfs"/rootfs/usr/local/bin/DepotDownloader
...
if [ "$arch" = "i686" ]; then
	...
	installersetup
	echo "export DOTNET_GCHeapHardLimitPercent=40" > "$installed_rootfs"/rootfs/etc/profile.d/dotnet.sh
	if [ "$BETA" -eq 1 ]; then
		print_yellow "WARNING: You are downloading beta version"
		sleep 1
